...
class DashboardController extends Controller
{
    ...
    public function activation()
    {
        ...
        if (file_exists(public_path().'/project/license.txt')){
            ...
            if ($license != ""){
                $activation_data = "<i style='color:darkgreen;' class='icofont-check-circled icofont-4x'></i><br><h3 style='color:darkgreen;'>আপনার ওয়েবসাইটের লাইসেন্স কি একটিভেটেড রয়েছে। যেকোন সমস্যায় কল করুনঃ ০1971111209</h3><br> আপনার লাইসেস্ন কি হল:  <b>".$license."</b>";
            }
        }
    ...
    {
        //return config('services.elitedesign.ocean');
        $purchase_code =  $request->pcode;
        $my_script =  'Newspaper';
        $my_domain = url('/');

        $varUrl = str_replace (' ', '%20', config('services.elitedesign.ocean').'purchase112662activate.php?code='.$purchase_code.'&domain='.$my_domain.'&script='.$my_script);
    ...
    public function movescript(){
        ...
        $files = public_path();
        $bkupname = 'elitedesignCart-By-EliteDesign-'.date('Y-m-d').'.zip';
