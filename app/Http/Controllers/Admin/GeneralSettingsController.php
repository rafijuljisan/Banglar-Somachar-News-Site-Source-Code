...
class GeneralSettingsController extends Controller
{
    protected $rules =
        [
            ...
            'print_logo' => 'mimes:jpeg,jpg,png,svg,webp',  // <--- ADD THIS,
            'photocard_frame' => 'mimes:jpeg,jpg,png,svg,webp',
        ];
    ...
    public function update(Request $request)
    {
        ...
        // --- Logic for Print Logo ---
        if ($request->hasFile('print_logo')) {
            $print_logo = $request->file('print_logo');
            // FIX: Remove spaces from filename to prevent URL errors (%20)
            $name = time() . str_replace(' ', '', $print_logo->getClientOriginalName());

            // FIX: Use public_path() to ensure it goes to project/public/assets/images
            $print_logo->move(public_path('assets/images'), $name);

            // Delete old image if it exists
            if ($data->print_logo && file_exists(public_path('assets/images/' . $data->print_logo))) {
                @unlink(public_path('assets/images/' . $data->print_logo));
            }

            $input['print_logo'] = $name;
        }
        // --- Logic for Photocard Frame ---
        if ($request->hasFile('photocard_frame')) {
            $frame = $request->file('photocard_frame');
            $name = time() . str_replace(' ', '', $frame->getClientOriginalName());
            $frame->move(public_path('assets/images'), $name);

            if ($data->photocard_frame && file_exists(public_path('assets/images/' . $data->photocard_frame))) {
                @unlink(public_path('assets/images/' . $data->photocard_frame));
            }
