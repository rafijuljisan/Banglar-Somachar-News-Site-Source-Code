...
@section('content')
<input type="hidden" id="headerdata" value="{{ __('ALBUM') }}">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ul class="links">
                    ...
                    <li>
                        <a href="{{ route('categories.index') }}">{{ __('Image Album') }}</a>
                    </li>
                </ul>
            </div>
        </div>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">
                    {{ __('You are about to delete this Image Album.') }}
                </p>
...
<script type="text/javascript">
    ...
    var table = $('#elitedesigntable').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route('image.album.datatables') }}',
        columns: [
    ...
    $(function () {
        $(".btn-area").append('<div class="col-sm-4 table-contents">' +'<a class="add-btn" data-href="{{route('image.album.create')}}" id="add-data" data-toggle="modal" data-target="#modal1">' 
        +'<i class="fas fa-plus"></i>{{__('Add New Album')}}' +'</a>' +'</div>');
    });
