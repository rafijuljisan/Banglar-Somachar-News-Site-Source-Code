...
@section('content')
<input type="hidden" id="headerdata" value="{{ __('POLLS') }}">
<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="heading">{{ __('Main Polls') }}</h4>
                <ul class="links">
                    <li>
                        <a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a>
                    </li>
                    <li><a href="javascript:;">{{ __('Manage Polls') }}</a></li>
                    <li>
                        <a href="{{ route('addPolls.index') }}">{{ __('Main Polls') }}</a>
                    </li>
                </ul>
            </div>
        </div>
                        ...
                        <table id="elitedesigntable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('Show On Home Page') }}</th>
                                    <th>{{ __('Poll Option View') }}</th>
                                    <th>{{ __('Actions') }}</th>
                                </tr>
                            </thead>
                        </table>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">
                    {{ __('You are about to delete this Poll.Everything under this poll will be deleted') }}.
                </p>
...
<script type="text/javascript">
    var table = $('#elitedesigntable').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route('addPolls.datatables') }}',
        columns: [
