      ...
      $(document).on('click','.status',function () {
        var link = $(this).attr('data-href');
            $.get( link, function(data) {
                ...
                $('#elitedesigntable').DataTable().ajax.reload();
                  $('.alert-danger').hide();
                  $('.alert-success').show();
...
$(document).on('submit','#elitedesignformdata',function(e){
  e.preventDefault();
  disablekey();
  // alert('ok');
  $('.gocover').show();
      $.ajax({
       ...
       success:function(data)
       {
          ...
          $('.alert-danger ul').html('');
            ...
            $('#elitedesignformdata input , #elitedesignformdata select , #elitedesignformdata textarea').eq(1).focus();
          }
          else
          {
            $('#elitedesigntable').DataTable().ajax.reload();
...
$(document).on('submit','#elitedesignformdataedit',function(e){
  e.preventDefault();
  disablekey();
  $('.gocover').show();
      $.ajax({
...
$(document).on('click','.submit-btn1',function(e){
  ...
  let fd = new FormData(document.getElementById("elitedesignformdata2"));
  fd.append('draft',draft);
  fd.append('description',description);

      $.ajax({
       method:"POST",
       url:$("#elitedesignformdata2").prop('action'),
       data:fd,
       contentType: false,
       cache: false,
       processData: false,
...
$(document).on('submit','#elitedesignform',function(e){
e.preventDefault();
    ...
    $.ajax({
     ...
     success:function(data)
     {
        ...
        if ((data.errors)) {
          ...
          elitedesignform.find('input , select , textarea').eq(1).focus();
        }
        else
        {
          $('.alert-success').show();
